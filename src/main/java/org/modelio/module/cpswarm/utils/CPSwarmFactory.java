...
public class CPSwarmFactory {
    ...
    public static StaticDiagram createProblemStatementDiagram(ModelElement element, String name, String description) {
        ...
        IUmlModel model = session.getModel();
        try{
            StaticDiagram diagram = model.createStaticDiagram("", element, ICPSWarmPeerModule.MODULE_NAME, CPSWarmStereotypes.PROBLEM_STATEMENT);
            session.getModel().getDefaultNameService().setDefaultName(diagram, I18nMessageService.getString("Ui.Name.ProblemStatement", element.getName()));
            session.getModel().createNote(MODELERMODULE_NAME, DESCRIPTION, diagram,description);
            return diagram;
        }catch(Exception e){
            return null;
